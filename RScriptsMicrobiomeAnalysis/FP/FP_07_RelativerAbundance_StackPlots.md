---
title: "FP_08_Relativer abundance stack plots"
...
# 7.1 Stack plot seperate accessions at family level
## Load data
...
levels(sample_data(FP_unnormalized_bac_ps_ForDA_clean)$Accession)
```
...
FP_unnormalized_bac_ps_ForDA_clean <- tax_glom(FP_unnormalized_bac_ps_ForDA_clean, taxrank = "Family", NArm = TRUE)
```
...
FP_unnormalized_bac_ps_ForDA_clean <- transform_sample_counts(FP_unnormalized_bac_ps_ForDA_clean, function(y) y / sum(y))

# Melt ps object
ps_df <- psmelt(FP_unnormalized_bac_ps_ForDA_clean)
```
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Family, data = ps_df, FUN = mean)

# Renormalize within each Accession × Soil_conditioning
split_idx <- interaction(ps_df_sum$Accession, ps_df_sum$Soil_conditioning)
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Family_plot, data = ps_df_sum, FUN = sum)
```

## Make stack plot

``` r
strip0 <- strip_themed(background_x = elem_list_rect(fill = c(rep("#E69F00", 1), rep("#D55E00", 3))))

ggplot(ps_df_sum, aes(x = Soil_conditioning, y = Abundance, fill = Family_plot)) +
...
![](FP_07_RelativerAbundance_StackPlots_files/figure-html/unnamed-chunk-8-1.png)<!-- -->
...
# 7.2 Stack plot seperate accessions at Order level
## Load data
...
FP_unnormalized_bac_ps_ForDA_clean <- tax_glom(FP_unnormalized_bac_ps_ForDA_clean, taxrank = "Order", NArm = TRUE)
```
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Order, data = ps_df, FUN = mean)
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Order_plot, data = ps_df_sum, FUN = sum)
```
...
ggplot(ps_df_sum, aes(x = Soil_conditioning, y = Abundance, fill = Order_plot)) +
 geom_bar(stat = "identity", width = 0.9) +
 facet_wrap2(~ Accession, 
             nrow = 1,
             strip = strip0) +
...
![](FP_07_RelativerAbundance_StackPlots_files/figure-html/unnamed-chunk-16-1.png)<!-- -->
...
# 7.3 Stack plot seperate accessions at Class level
## Load data
...
FP_unnormalized_bac_ps_ForDA_clean <- tax_glom(FP_unnormalized_bac_ps_ForDA_clean, taxrank = "Class", NArm = TRUE)
```
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Class, data = ps_df, FUN = mean)
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Class_plot, data = ps_df_sum, FUN = sum)
```
...
ggplot(ps_df_sum, aes(x = Soil_conditioning, y = Abundance, fill = Class_plot)) +
 geom_bar(stat = "identity", width = 0.9) +
 facet_wrap2(~ Accession, 
             nrow = 1,
             strip = strip0) +
...
![](FP_07_RelativerAbundance_StackPlots_files/figure-html/unnamed-chunk-24-1.png)<!-- -->
...
# 7.4 Stack plot seperate accessions at Phylum level
## Load data
...
FP_unnormalized_bac_ps_ForDA_clean <- tax_glom(FP_unnormalized_bac_ps_ForDA_clean, taxrank = "Phylum", NArm = TRUE)
```
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Phylum, data = ps_df, FUN = mean)
...
ps_df_sum <- aggregate(Abundance ~ Accession + Soil_conditioning + Phylum_plot, data = ps_df_sum, FUN = sum)
```
...
ggplot(ps_df_sum, aes(x = Soil_conditioning, y = Abundance, fill = Phylum_plot)) +
 geom_bar(stat = "identity", width = 0.9) +
 facet_wrap2(~ Accession, 
             nrow = 1,
             strip = strip0) +
...
![](FP_07_RelativerAbundance_StackPlots_files/figure-html/unnamed-chunk-32-1.png)<!-- -->
